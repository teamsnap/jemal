...
class ViewAllFavoritedEmails extends Component {
    render() {
        ...
        const emails = this.props.getFavoritedEmails && this.props.getFavoritedEmails.getFavoritedEmails;
        const count = this.props.getFavoritedEmailsCount && this.props.getFavoritedEmailsCount.getFavoritedEmailsCount;
        ...
        if (emails) {
            renderEmails = emails.map( ( { title, isDraft, hasBeenSent, isApproved, updatedAt, createdAt, updatedById, createdById, screenshot, _id, favorited } ) => {
                return <EmailCard key={_id} _id={_id} title={title} link={`/email/edit/${_id}`} email={true} isDraft={isDraft} hasBeenSent={hasBeenSent} isApproved={isApproved} updatedAt={updatedAt} createdAt={createdAt} updatedById={updatedById} createdById={createdById} image={screenshot} favorited={favorited} />
            });
        }
              ...
              <Grid container spacing={24}>
                <Grid item sm={12}>
                  <Paper style={styles.paper}>
                    <Typography variant="display1" component="h1" style={styles.heading}>Viewing all emails</Typography>
                    <Paper style={styles.paper}>
                      ...
                      <Grid container spacing={24}>
                        ...
                        {count && count.count > limit ? (
                            <Pagination count={count && count.count} limit={limit} link="/email/favorited/view/page/" />
                        ) : (
                            null
                        )}
                    </Paper>
