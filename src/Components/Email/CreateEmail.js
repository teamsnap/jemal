...
class CreateEmailView extends Component {
  ...
  render() {
    ...
    const baseTemplates = this.props.getBaseTemplateEmails && this.props.getBaseTemplateEmails.getBaseTemplateEmails;

    let renderBaseTemplates;

    if (baseTemplates) {
        renderBaseTemplates = baseTemplates.map( ( { title, mjmlSource, _id } ) => {
            return <MenuItem key={_id} value={mjmlSource}>{title}</MenuItem>
        });
    }
    ...
    return (
      <div>
        <Card style={styles.card}>
          <CardContent>
            <form action="/">
              <Typography variant="headline" component="h1">Create a new email</Typography>
            ...
            <CardActions>
              <Button variant="raised" color="primary" size="small" onClick={this.createEmail}>Create</Button>
...
const createEmail = gql`
  mutation createEmail($title: String!, $mjmlSource: String $baseTemplate: Boolean, $duplicatedFrom: String, $folderPath: String, $favorited: Boolean, $isApproved: Boolean, $hasBeenSent: Boolean, $isDraft: Boolean, $organizationId: String!) {
      createEmail(title: $title, mjmlSource: $mjmlSource, baseTemplate: $baseTemplate, duplicatedFrom: $duplicatedFrom, folderPath: $folderPath, favorited: $favorited, isApproved: $isApproved, hasBeenSent: $hasBeenSent, isDraft: $isDraft, organizationId: $organizationId) {
          _id
      }
  }
`;
