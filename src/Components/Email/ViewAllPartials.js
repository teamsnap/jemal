...
class ViewAllPartials extends Component {
    render() {
        ...
        const emailPartials = this.props.getAllEmailPartials && this.props.getAllEmailPartials.getAllEmailPartials
        const count = this.props.getEmailPartialsCount && this.props.getEmailPartialsCount.getEmailPartialsCount;
        ...
        if (emailPartials) {
            renderEmailPartials = emailPartials.map( ( { title, _id } )=>{
                return <EmailCard key={_id} _id={_id} title={title} link={`/email/partials/edit/${_id}`} />
            });
        }
              ...
              <Grid container spacing={24}>
                <Grid item sm={12}>
                  <Paper style={styles.paper}>
                    <Typography variant="display1" component="h1">Viewing all email partials</Typography>
                    <Paper style={styles.paper}>
                      <Link to="/email/partial/create"><Button variant="raised" color="primary" size="large">New email partial</Button></Link>
                      <Grid container spacing={24}>
                        {renderEmailPartials}
                      </Grid>
                        {count && count.count > limit ? (
                            <Pagination count={count && count.count} limit={limit} link="/email/partials/view/page/" />
                        ) : (
                            null
                        )}
                    </Paper>
