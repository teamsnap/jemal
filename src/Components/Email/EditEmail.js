...
class EditEmailView extends Component {
    ...
    componentWillReceiveProps(newProps) {
        if (!newProps.loading) {
            const { title, mjmlSource, baseTemplate, duplicatedFrom, folderPath, favorited, isApproved, hasBeenSent, isDraft } = !newProps.getCurrentEmail.loading && newProps.getCurrentEmail.getCurrentEmail
            this.setState({
                title,
                mjmlSource,
                baseTemplate,
    ...
    componentWillMount() {
        if (!this.props.loading) {
            const { title, mjmlSource, baseTemplate, duplicatedFrom, folderPath, favorited, isApproved, hasBeenSent, isDraft } = !this.props.getCurrentEmail.loading && this.props.getCurrentEmail.getCurrentEmail
            this.setState({
                title,
                mjmlSource,
                baseTemplate,
    ...
    render() {
        ...
        const email = !this.props.getCurrentEmail.loading && this.props.getCurrentEmail.getCurrentEmail
            ...
            <div style={styles.root}>
                    ...
                    <Grid container spacing={24}>
                        <Grid item sm={12}>
                            <Paper style={styles.paper}>
                                <Grid container spacing={24}>
                                    <Grid item sm={2}>
                                        <Typography variant="display1" component="h1">Email Editor</Typography>
                                    </Grid>
                                    <Grid item sm={3}>
                                        <TextField
                                          name="title"
                                    ...
                                    <Grid item sm={4}>
                                       ...
                                       />
                                        <FormControlLabel
                                         ...
                                         label="Base Template"
                                       />
                                       {this.state.favorited ? <HeartIcon color="primary" onClick={this.toggleFavorite(true)} /> :  <HeartOutlineIcon color="secondary" onClick={this.toggleFavorite(false)} />}
                                    </Grid>
                                    <Grid item sm={3}>
                                        {this.state.copied ? <span onClick={() => this.setState({ copied: false })} style={{ position: 'absolute', backgroundColor: 'white', padding: 20, borderRadius: 8, right: 200, top: 20, zIndex: 100 }}>Copied. [Close]</span> : null}
                                        <Button variant="raised" color="primary" size="small" onClick={this.editEmail}>Save</Button>
                                        <CopyToClipboard text={email.urlPreview} onCopy={() => this.setState({ copied: true })}>
                                            <Button variant="raised" color="primary" size="small">Copy HTML</Button>
                                        </CopyToClipboard>
                ...
                <Grid container spacing={24}>
                    ...
                    <Grid item sm={6} style={{ paddingRight: 40 }}>
                        <Iframe style={styles.iframe} content={email.urlPreview} title={email.title} />
                    </Grid>
                </Grid>
            </div>
        );
...
const editEmail = gql`
  mutation editEmail($_id: String!, $title: String!, $mjmlSource: String! $baseTemplate: Boolean, $folderPath: String, $favorited: Boolean, $isApproved: Boolean, $hasBeenSent: Boolean, $isDraft: Boolean, $organizationId: String!) {
      editEmail(_id: $_id, title: $title, mjmlSource: $mjmlSource, baseTemplate: $baseTemplate, folderPath: $folderPath, favorited: $favorited, isApproved: $isApproved, hasBeenSent: $hasBeenSent, isDraft: $isDraft, organizationId: $organizationId) {
          _id
      }
  }
`;
