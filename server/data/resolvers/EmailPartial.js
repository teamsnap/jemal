...
const EmailPartialResolver = {
  Query: {
    getEmailPartialsCount: async (root, { _id }, { user }) => {
      ...
      const emailPartialCount = await EmailPartial.count(
        { organizationId: _id },
        (err, org) => {
    ...
    getCurrentEmailPartial: async (root, { _id }, { user }) => {
      ...
      const emailPartialsFound = await EmailPartial.findOne(
        { _id },
        (err, org) => {
          if (err) console.error(err);
        }
  ...
  Mutation: {
    duplicateEmailPartial: async (root, { _id }, { user }) => {
      ...
      const {
        ...
        organizationId,
        folderPath
      } = await EmailPartial.findOne({ _id });
    ...
    createEmailPartial: async (
      roots,
      { title, description, mjmlSource, organizationId, folderPath },
      { user }
    ) => {
      if (!user._id) throw new Error('Must be logged in');
      if (!organizationId)
